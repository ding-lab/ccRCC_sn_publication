...
# Author:       Yige Wu
# Description:  Bar plot showing the bulk RNA-seq expression of RCC4 cells with and without MXI1 knockdown
...
colors_byline <- RColorBrewer::brewer.pal(n = 6, name = "Set2")[c(1, 3)]
names(colors_byline) <- c("sh-NC", "sh-MXI1-C1")
...
# ## write plot data
# write.table(x = plot_data_df, file = "../../plot_data/F3f.SourceData.tsv", quote = F, sep = "\t", row.names = F)

# input plot data ---------------------------------------------------------
plot_data_df <- fread(data.table = F, input = "../../plot_data/F3f.SourceData.tsv")
...
file2write <- paste0(dir_out,"F3f.Barplot.gene_expression_MXI1_HK2", ".pdf")
