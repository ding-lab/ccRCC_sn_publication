...
# ## save plot data
# write.table(x = plotdata_df, file = "../../plot_data/F3c.DownDEGs.SourceData.tsv", quote = F, sep = "\t", row.names = F)

# input plot data ---------------------------------------------------------
plotdata_df <- fread(data.table = F, input = "../../plot_data/F3c.DownDEGs.SourceData.tsv")
...
file2write <- paste0(dir_out,"F3c.Dotplot.ORA_down_DEGs_tumorcells_vs_pt", ".pdf")
