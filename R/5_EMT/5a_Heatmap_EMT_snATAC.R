...
# Author:       Yige Wu
# Description:  Heatmap showing gene activity of the epithelial and mesenchymal marker genes for 38 tumor clusters and 9 PT clusters (> 50 cells) using snATAC-seq data
...
avgexp_df1 <- fread(input = "../../data/ATAC.AverageGeneActivity_ByTumorPTLOHSubcluster.v2.20210917.tsv.gz", data.table = F)
avgexp_df2 <- fread(input = "../../data/ATAC.AverageGeneActivity.ccRCC_markers.Surface.20210928.tsv.gz", data.table = F)
# ## input by cluster enrichment assignment
emt_scores_df <- fread(data.table = F, input = "../../data/ATAC.Pathwayscores_byTumorcluster.MSigDB.Hallmark.tsv.gz")
epi_group_df <- fread(data.table = F, input = "../../data/Tumorcluster_EpithelialGroup.20210929.v1.tsv.gz")
## input score pre-calculated
epi_scores_df <- fread(data.table = F, input = "../../data/ATAC.EpithelialScore.tsv.gz")
s12_scores_df <- fread(data.table = F, input = "../../data/ATAC.Tumorcluster_PTS12Score.tsv.gz")
s3_scores_df <- fread(data.table = F, input = "../../data/ATAC.Tumorcluster_PTS3Score.tsv.gz")
## input clinical info
clinical_specimen_df <- fread(data.table = F, input = "../../data/Specimen_Clinicl_Data.20210706.v1.tsv")
...
## write plot data
write.table(x = plot_data_mat, file = "../../plot_data/F5a.snATAC.SourceData.tsv", quote = F, sep = "\t", row.names = F)
...
file2write <- paste0(dir_out, "F5a_Heatmap_EMT_snATAC", ".pdf")
