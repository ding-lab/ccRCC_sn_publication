...
# Author:       Yige Wu
# Description:  Heatmap showing gene expression of the epithelial and mesenchymal marker genes for 90 tumor clusters and 9 proximal tubule (PT) clusters (> 50 cells) using snRNA-seq data
...
avgexp_df <- fread(input = "../../data/AverageExpression_ByTumorPTLOHSubcluster.20210903.v1.tsv.gz", data.table = F)
...
emt_scores_df <- fread(data.table = F, input = "../../data/Tumorcluster_EpithelialGroup.20210929.v1.tsv.gz")
s12_scores_df <- fread(data.table = F, input = "../../data/Tumorcluster_PTS12Score.tsv.gz")
s3_scores_df <- fread(data.table = F, input = "../../data/Tumorcluster_PTS3Score.tsv.gz")
## input clinical info
...
colanno_obj = HeatmapAnnotation(CellType_CellState = anno_simple(x = colanno_df$epithelial_group, col = colors_tumorgroup_sim[colanno_df$epithelial_group], height = unit(0.75, "cm")),
...
file2write <- paste0(dir_out, "F5a_Heatmap_EMT_snRNA", ".pdf")
